...
```{r}
m = read.csv("csvsFinales/dataset.csv")
head(m)
```
...
Graficar relación lineal entre cada predictor y la variable de respuesta.

```{r}
par(mfrow = c(2,2))
plot(X1, reg$residuals, col = "blue", ylab = "Planta", xlab = "NOX")
abline(h=0, col = "red")
plot(X3, reg$residuals, col = "blue", ylab = "Planta", xlab = "PM10")
abline(h=0, col = "red")
plot(X5, reg$residuals, col = "blue", ylab = "Planta", xlab = "SR")
abline(h=0, col = "red")
plot(X8, reg$residuals, col = "blue", ylab = "Planta", xlab = "WDR")
abline(h=0, col = "red")
```

Los residuos parecen distribuirse aleatoriamente alrededor de la media cero, por lo que se podría concluir que la variable dependiente (predictora) tiene relación lineal con cada una de sus variables independientes (explicativas).
